...
Image ImgSource::getImage()
{
  ...
  if (readCamera)  // Read from camera
  {
    ...
    path cameraColorsPath =
        (getenv("HOME")) + string("/Eclipse/Target_Detection/colors.txt");
  ...
  else  // Read from filesystem
  {
    ...
    else  // Read series of images
    {
      ...
      if (filePaths
              .empty())  // Check to see if there are any more images to import
      {
        img.valid = false;
        return img;
      }
      ...
      Mat filesystemImage;
      filesystemImage =
          imread(this->filePaths.front(), CV_LOAD_IMAGE_COLOR);  // Read
...
// Loop through the parent folder in the filesystem to add all image paths to
// queue Makes directories
void ImgSource::getFileImages()
{
  recursive_directory_iterator end_itr;

  for (recursive_directory_iterator itr(this->folderPath); itr != end_itr;
       ++itr)
  {
    // Path strings
    ...
    if (is_directory(itr->path()))
    {
      ...
      outputDirectoryPath.replace(index, 5,
                                  "Output");  // Replace "Input" with "Output
    ...
    color_data imgColors = this->getFileColors(
        itr->path());  // Read colors from text file in folder. Inefficient, but
                       // easy and only used during testing from filesystem.

    // If not a jpeg, skip to next file
    if (currentFileName.find(".jpg") == string::npos) continue;
