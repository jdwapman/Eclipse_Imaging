...
vector<vector<Point> > Tarp::findTarpContours(const Mat& imgHSV)
{
  ...
  Scalar low = {(double)this->hsv_low[0], (double)this->hsv_low[1],
                (double)this->hsv_low[2]};
  Scalar high = {(double)this->hsv_high[0], (double)this->hsv_high[1],
                 (double)this->hsv_high[2]};
  TickMeter tm;
  tm.start();
  ...
  findContours(cpuThresh, contours, hierarchy, RETR_TREE, CHAIN_APPROX_NONE,
               Point(0, 0));
  contours_approx.resize(contours.size());

  // Appromiximate contours. Typically 1% - 5% error is typical.
  // Additionally, this creates closed loops of all contours. (Needed to find
  // center/average color)
  for (size_t k = 0; k < contours.size(); k++)
  {
    approxPolyDP(Mat(contours[k]), contours_approx[k],
                 0.05 * arcLength(contours[k], true), true);
  }
...
vector<tuple<Scalar, Scalar, unsigned int> > Tarp::findTarpMeans(
    vector<vector<Point> > tarpContours, vector<Mat> splitImgHSV,
    vector<bool>& tarpValid)
{
  ...
  for (unsigned int i = 0; i < tarpContours.size(); i++)
  {
    if (tarpValid[i] == true)
    {
      Mat mask(splitImgHSV[0].rows, splitImgHSV[0].cols, CV_8UC1,
               Scalar(0));  // Initialize

      drawContours(mask, tarpContours, i, Scalar(255), -1,
                   8);  // Draw filled in mask
      ...
      tarpMeans[i] =
          make_tuple(mean[0], stddev[0],
                     i);  // Gets average value of the points inside the mask
    }
  }
...
vector<tuple<double, unsigned int> > Tarp::findTarpHist(
    vector<vector<Point> > tarpContours, vector<Mat> splitImgHSV,
    vector<bool>& tarpValid)
{
  ...
  for (unsigned int i = 0; i < tarpContours.size(); i++)
  {
    if (tarpValid[i] == true)
    {
      ...
      calcHist(&splitImgHSV[0], 1, 0, mask, hist, 1, &histSize, &histRange,
               uniform, accumulate);
      ...
      //			cout << "maxLocY: " << maxLoc.y  << endl;
      ////Dominant frequency of the contour
      ...
      int color_offset =
          abs(this->hsv_ideal[0] - maxLoc.y);  // Checks difference in HSV value
      ...
      tarpDominantColor[i] = make_tuple(
          maxLoc.y, i);  // Gets average value of the points inside the mask
    }
  }
...
vector<tuple<double, unsigned int> > Tarp::findTarpAreas(
    vector<vector<Point> > tarpContours, vector<bool>& tarpValid)
{
...
vector<tuple<unsigned int, unsigned int> > Tarp::findTarpVertices(
    vector<vector<Point> > tarpContours, vector<bool>& tarpValid)
{
...
void filterSideLengths(vector<vector<Point> > tarpContours,
                       vector<bool>& tarpValid)
{
  for (unsigned int i = 0; i < tarpContours.size(); i++)
  {
    if (tarpValid[i])
    {
      ...
      if (*min_element(lengths.begin(), lengths.end()) <
          0.6 * (*max_element(lengths.begin(), lengths.end())))
      {
        tarpValid[i] = false;
      }
    }
...
void filterEdgeContrast(vector<vector<Point> > tarpContours,
                        vector<Mat> splitImgHSV, vector<bool>& tarpValid)
{
  for (unsigned int i = 0; i < tarpContours.size(); i++)
  {
    if (tarpValid[i] == true)
    {
      ...
      for (unsigned int k = 0; k < 3; k++)
      {
        ...
        Mat innerMask(splitImgHSV[k].rows, splitImgHSV[k].cols, CV_8UC1,
                      Scalar(0));  // Initialize

        drawContours(innerMask, tarpContours, i, Scalar(255), -1,
                     8);  // Draw filled in mask
        ...
        Mat outerMask(splitImgHSV[k].rows, splitImgHSV[k].cols, CV_8UC1,
                      Scalar(0));  // Initialize
        drawContours(outerMask, largeContour, 0, Scalar(255), -1,
                     8);  // Draw filled in mask. Only 1 contour
        ...
        if (abs(outerMean.val[0] - innerMean.val[0]) <
            5)  // Reject if there isn't enough contrast
        {
          tarpValid[i] = false;
        }
      }
...
void Tarp::findBestTarp(Mat& imgHSV, vector<Mat>& splitImgHSV,
                        vector<Point>& bestTarp)
{
  // Get tarp contours
  ...
  vector<tuple<unsigned int, unsigned int> > tarpVertices =
      findTarpVertices(tarpContours, tarpValid);
  ...
  // Get tarp areas
  vector<tuple<double, unsigned int> > tarpAreas =
      findTarpAreas(tarpContours, tarpValid);

  // Get tarp histogram
  // vector< tuple<double, unsigned int> > tarpDominantColor =
  // findTarpHist(tarpContours, splitImgHSV, tarpValid);

  // Get tarp mean, stddev
  // vector< tuple<Scalar, Scalar, unsigned int> > tarpMeanStddev =
  // findTarpMeans(tarpContours, splitImgHSV, tarpValid);
  ...
  // Base Case 0
  if (numContours == 0) bestTarp = tarp;  // Return empty vector of points
  ...
  for (unsigned int i = 0; i < numContours; i++)
  {
    if (tarpValid[get<1>(tarpAreas[i])] == true)
    {
      ...
      //			//Write information
      //			cout << "Area: " <<
      //get<0>(tarpAreas[get<1>(tarpAreas[i])]) << endl; 			cout << "Vertices: " <<
      //get<0>(tarpVertices[get<1>(tarpAreas[i])]) << endl; 			cout << "Convex: "
      //<< tarpConvexity[get<1>(tarpAreas[i])] << endl; 			cout << "Dominant Color:
      //" << (get<0>(tarpDominantColor[get<1>(tarpAreas[i])])) << endl; 			cout <<
      //"Index: " << get<1>(tarpAreas[i]); 			cout << endl;
  ...
  // draw contours
  //	Mat drawmat = Mat::zeros(splitImgHSV[0].rows,splitImgHSV[0].cols,
  //CV_8UC1);
  //
  ...
  //				drawContours( drawmat, tarpContours, i, color, 1,
  //8);
  //			}
  //			else
  //			{
